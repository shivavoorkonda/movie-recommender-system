...
How Content-Based Filtering Works:
  1. Build a "profile" for each movie from its features (here: genres + title keywords).
...
Content-Based is great for the cold-start problem (new users with no history).
"""
...
class ContentBasedModel:
    ...
    def _build_feature_string(self, row: pd.Series) -> str:
        ...
        We repeat genre tokens to boost their importance relative to
        title words (a simple but effective weighting trick).
        """
        genres  = " ".join(row["genre_list"]) if isinstance(row.get("genre_list"), list) else str(row.get("genres", ""))
        genres  = genres.replace("|", " ").replace("-", " ")
        # Repeat genres 3× to give them more weight vs. title words
    ...
    def get_user_profile_recommendations(self, user_id: int, ratings_df: pd.DataFrame,
                                          n: int = 10, min_rating: float = 4.0) -> pd.DataFrame:
        """
        ...
        Approach:
          ...
          4. Exclude already-rated movies, return top-N.
